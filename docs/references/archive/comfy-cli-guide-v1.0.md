...
# 检查并安装 comfy-cli
if ! /comfy/mnt/venv/bin/comfy --version &> /dev/null; then
    echo "Installing comfy-cli..."
    /comfy/mnt/venv/bin/pip install comfy-cli
fi

# 设置默认工作区
echo "Setting default workspace to /basedir..."
/comfy/mnt/venv/bin/comfy set-default /basedir 2>/dev/null || true
...
由于 comfy-cli 本身不提供 REST API，需要通过 custom node 将命令暴露为 HTTP 端点。有两种实现方案：
...
*本文档由 Zero (零号) 生成，基于 comfy-cli v0.1.0+ 和 ComfyUI-Nvidia-Docker 环境。*
